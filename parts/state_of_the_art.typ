...
==== Timers in the RISC-V ISA
The RISC-V ISA (both in its 32-bit and 64-bit variants) defines two 64-bit
"machine timer" memory-mapped CSRs: `mtime` and `mtimecmp`. As stated in the
specification, #quote[`mtime` must increment at constant frequency, and the
  platform must provide a mechanism for determining the period of an `mtime`
  tick. The `mtime` register will wrap around if the count overflows]. To signal
the deadline has been met, #quote[[t]he interrupt remains posted until
  `mtimecmp` becomes greater than `mtime`] @RISCVUnprivileged[chap. 3.2].

// timer interrupts -> disabled, etc.


==== Timers in the Sinclair ZX Spectrum
