...
After the handler finishes, the interrupt pin must be reset#footnote[In the case
  ...
  @subsec:z80-peripherals.] before re-enabling interrupts, and returning from
the routine. A `RET` instruction#footnote[The `RET` instruction pops the top of
  the stack and stores it into the PC.] may be used to resume execution, but the
Z80 ISA offers some alternatives with additional functionality for handling
...
setting the `MTIE` flag in the `mie` register @RISCVUnprivileged[chap. 3.2].
...
=== Z80 peripherals <subsec:z80-peripherals>
